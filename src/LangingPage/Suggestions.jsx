      ...
      <main className="max-w-[1500px] mx-auto px-6 md:px-12 py-16 relative z-10">
        ...
        <header className="mb-16 border-b border-[#023347]/5 pb-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
          <div className="flex flex-col items-start text-left">
            ...
            <h1 className={`text-5xl font-semibold leading-tight transition-all duration-[1200ms] ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'}`}>
        ...
        <div className={`relative bg-white/[0.02] backdrop-blur-[4px] border border-black/5 rounded-[2rem] p-10 md:p-16 transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
          ...
          <div className="space-y-12">
            <div className="flex flex-col gap-y-12 text-left">
              ...
              {/* Title Field */}
              <div>
                <label className="text-[10px] font-bold tracking-widest uppercase text-[#023347]/50 mb-3 block">Suggestion Title</label>
                <input 
                  type="text" 
                  name="title" 
                  value={form.title} 
                  ...
                  className={`w-full bg-transparent border-b py-4 font-sans text-xl outline-none transition-colors placeholder:text-[#023347]/20 ${errors.title ? 'border-red-400' : 'border-[#023347]/10 focus:border-[#D4AF37]'}`} 
                />
                {errors.title && <p className="text-[10px] text-red-500 font-bold mt-2 uppercase tracking-tighter">{errors.title}</p>}
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className={`w-full bg-transparent border rounded-2xl p-6 font-sans text-lg outline-none transition-colors min-h-[250px] resize-none placeholder:text-[#023347]/20 ${errors.description ? 'border-red-400' : 'border-[#023347]/10 focus:border-[#D4AF37]'}`} 
                />
                {errors.description && <p className="text-[10px] text-red-500 font-bold mt-2 uppercase tracking-tighter">{errors.description}</p>}
              </div>
            </div>
            ...
            <div className="bg-[#D4AF37]/5 border border-[#D4AF37]/20 rounded-2xl p-6 flex flex-col items-center text-center">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-4 h-4 rounded-full bg-[#D4AF37] text-white text-[10px] flex items-center justify-center font-bold">!</div>
                <span className="text-[10px] font-bold tracking-widest uppercase text-[#B8860B]">Discliamer</span>
              </div>
              <p className="text-[13px] text-[#023347]/70 leading-relaxed max-w-xl">
            ...
            <div className="flex justify-end pt-6">
              <button 
                ...
                className={`bg-[#023347] text-white px-20 py-5 rounded-2xl text-[11px] font-bold tracking-[0.2em] uppercase transition-all duration-500 hover:bg-[#D4AF37] hover:shadow-2xl hover:shadow-[#D4AF37]/20 active:scale-95 ${isShaking ? 'animate-shake bg-red-500' : ''}`}
              >
                Submit
              </button>
            </div>
